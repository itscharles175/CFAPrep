...
### CI / build
- **M19 — Visual-regression gate is a false-green.** 🟣 ✓ `tests/visual-baselines/` is gitignored; `visual-regression.mjs:147` writes missing baselines instead of failing → every fresh CI run self-seeds and passes. **Fix:** commit baselines (or fail on missing baseline in CI). *(Cosmetic gate → MEDIUM.)*
