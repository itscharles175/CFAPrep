...
- `npm run cfa:source:audit` to confirm private CFA source bundles, PDFs, EPUBs, OCR derivatives, and extracted text are not present in tracked files or release artifact roots.
- `npm run stack:audit` for route-manifest structure, offline-critical coverage, PWA fallback wiring, artifact denylist checks, screenshot-route coverage, and release-gate manifest drift.
...
Level III uses the eight active topics in the current QuantVault taxonomy: Ethics, Asset Allocation, Portfolio Construction, Performance Measurement, Derivatives And Risk Management, Portfolio Management Pathway, Private Markets Pathway, and Private Wealth Pathway.

The CFA Source Vault is a private local study corpus. Standard vault exports omit source text; `.qvsource` bundles and extracted source text must remain in the ignored local app-data source vault or browser IndexedDB, never in tracked files, `public`, `docs`, `dist`, release reports, screenshots, or CI artifacts. `QV_SOURCE_VAULT_DIR` may override the private local bundle directory. `VITE_*` environment values are public client configuration and must not be treated as secrets.
