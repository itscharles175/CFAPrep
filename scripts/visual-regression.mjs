 ...
 * BOTH the light and dark themes, then pixel-diffs each frame against a committed
 * baseline. The goal is a cheap drift alarm for the design system: if a token,
 * primitive, or layout shifts unintentionally, the matching baseline stops
 * matching and CI fails.
 *
 ...
 *
 * BASELINES (committed): tests/visual-baselines/<route>-<theme>.png.
...
import { viewports } from './qa-helpers.mjs';
...
// Curated, deterministic surfaces. /style is the token gallery (the richest
...
const TARGET_ROUTES = CURATED_ROUTE_IDS.map((id) => {
 ...
 */
async function compareToBaseline({ id, theme, screenshot }) {
  const name = `${id}-${theme}`;
...
// A fresh context per theme isolates the seeded localStorage['qv-theme'] so the
// bootstrap applies the right `data-theme` before first paint (no theme bleed).
    ...
    await withThemeContext(theme, async (page) => {
      ...
      for (const route of TARGET_ROUTES) {
        ...
        try {
          ...
          const result = await compareToBaseline({ id: route.id, theme, screenshot });
          results.push({ ...result, routeId: route.id, theme, path: route.path, url, durationMs: Date.now() - startedAt });
          ...
          results.push({ outcome: 'error', routeId: route.id, theme, path: route.path, url, reason: message, durationMs: Date.now() - startedAt });
