#!/usr/bin/env node
/*
 * Build the open-notebook API and worker into platform-native binaries
 * via PyInstaller, then copy it into `electron/resources/services/`
 * where electron-builder's `extraResources` will pick it up and ship
 ...
 *   - Windows:  electron/resources/services/open-notebook/open-notebook.exe
 *               electron/resources/services/open-notebook/open-notebook-worker.exe
 *   - macOS:    electron/resources/services/open-notebook/open-notebook
 *               electron/resources/services/open-notebook/open-notebook-worker
 *   - Linux:    electron/resources/services/open-notebook/open-notebook
 *               electron/resources/services/open-notebook/open-notebook-worker
 *
 * Production runtime resolves these files from process.resourcesPath/services.
 */
...
const WORKER_SPEC_NAME = 'open-notebook-worker';
const WORKER_ENTRY = join(REPO_ROOT, '.open-notebook-worker-entry.py');
const OUT_DIR = join(REPO_ROOT, 'electron', 'resources', 'services', 'open-notebook');
...
const workerEntryValue = execSync(
  `${pythonCmd} -c "import importlib.metadata as m; e=[x for x in m.entry_points(group='console_scripts') if x.name=='surreal-commands-worker']; print((e[0].module + ':' + e[0].attr) if len(e)==1 and e[0].attr else '')"`,
  { cwd: REPO_ROOT, encoding: 'utf8', shell: true },
).trim();
const workerEntryMatch = /^([A-Za-z_]\w*(?:\.[A-Za-z_]\w)*):([A-Za-z_]\w*)$/.exec(workerEntryValue);
ensure(
  workerEntryMatch,
  `Expected exactly one importable surreal-commands-worker entry point, received ${workerEntryValue || '<none>'}`,
);
const [, workerModule, workerFunction] = workerEntryMatch;
...
WORKER_ENTRY = r'${WORKER_ENTRY.replace(/\\/g, '/')}'
...
writeFileSync(
  WORKER_ENTRY,
  `from ${workerModule} import ${workerFunction} as _entry\n\nif __name__ == '__main__':\n    raise SystemExit(_entry())\n`,
);
...
try {
  ...
  rmSync(WORKER_ENTRY, { force: true });
}

// Copy result into electron/resources/services/open-notebook/.
const exeName = process.platform === 'win32' ? `${PYINST_SPEC_NAME}.exe` : PYINST_SPEC_NAME;
const workerExeName = process.platform === 'win32' ? `${WORKER_SPEC_NAME}.exe` : WORKER_SPEC_NAME;
const built = join(PYINST_DIST, exeName);
const builtWorker = join(PYINST_DIST, workerExeName);
ensure(exists(built), `PyInstaller did not produce ${built}.`);
ensure(exists(builtWorker), `PyInstaller did not produce ${builtWorker}.`);

const dest = join(OUT_DIR, exeName);
const workerDest = join(OUT_DIR, workerExeName);
if (existsSync(dest)) rmSync(dest);
if (existsSync(workerDest)) rmSync(workerDest);
const { copyFileSync, chmodSync } = await import('node:fs');
copyFileSync(built, dest);
copyFileSync(builtWorker, workerDest);
if (process.platform !== 'win32') {
  chmodSync(dest, 0o755);
  chmodSync(workerDest, 0o755);
}
...
const workerProvenance = await recordSidecarProvenance({
  service: 'open-notebook worker binary',
  binaryPath: workerDest,
  source: onbSourceHead ? `spike/open-notebook@${onbSourceHead}` : 'scripts/build-onb-binary.mjs',
  optional: true,
});
...
console.log(`✓ Bundled open-notebook worker -> ${workerDest.split(sep).slice(-4).join(sep)}`);
console.log(`  Provenance -> ${workerProvenance.sha256.slice(0, 12)}… (${workerProvenance.size} bytes)`);
console.log(`  Electron release builds will now include it through extraResources.`);
