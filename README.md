...
- **Tauri-native folder picker** for PDF folders (Rust commands
  `cfa_pick_folder`, `cfa_list_pdfs`, `cfa_read_pdf_bytes` + browser-side
...
forecast. Surfaced as a panel on the CFA dashboard *and* as the dedicated
...
# Tauri shell:
cd src-tauri && cargo test    # 5 Rust unit tests
cd .. && npx tauri build      # release bundle
```

## Run dev

Web-only dev (no native folder picker, no Tauri-CORS bypass):
...
Desktop dev (Tauri shell auto-supervises the sidecars):

```bash
npx tauri dev
```

Note: stop any spike servers on `:8000` / `:5055` first — the supervisor
binds those ports. See `docs/SPIKE-FINDINGS.md`.
...
- **LM Studio** (recommended): serve `gemma-4-e4b-it` on
  ...
  browser fetches in web dev. The Tauri shell is CORS-immune.
...
- The Tauri shell never leaves the user's machine.
