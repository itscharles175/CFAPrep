...
describe('api.request — error mapping + validation boundary (7.2)', () => {
  ...
  it('passes a successful JSON body straight through (no validate)', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ ok: true, service: 'lsat-backend', version: '0.9.0' })),
    );
  ...
  it('attaches the in-memory local API token to typed API requests', async () => {
    window.__STUDYVAULT_LSATLAB_LOCAL_API_TOKEN__ = 'run-token';
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ ok: true, service: 'lsat-backend', version: '0.9.0' }));
  ...
  it('maps a string `detail` error body to ApiError.message + status', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ detail: 'Section not found' }, { ok: false, status: 404 })),
    );
  ...
  it('preserves a structured `detail` object on ApiError.detail (D1 gate)', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ detail }, { ok: false, status: 409 })));
    let caught: unknown;
    try {
      await api.health();
    } catch (e) {
  ...
  it('returns undefined for a 204 no-content response', async () => {
    ...
    await expect(api.blindReview(1, { br_answer: 'A', confidence: 'sure' })).resolves.toBeUndefined();
  });
  ...
  it('does not fake success when persistent bank bulk tagging fails', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ detail: 'database locked' }, { ok: false, status: 503 })),
    );

    await expect(api.bankBulkTag({ question_ids: [1], q_type: 'Flaw' })).rejects.toMatchObject({
      name: 'ApiError',
      status: 503,
  ...
  it('passes the feedback cohort evidence window through the API query', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('/api/analytics/feedback-cohorts?days=90');
  });
  ...
  it('passes feedback outcome filters through the API query', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('/api/analytics/feedback-outcomes?');
  ...
  it('throws ApiValidationError when a Tutor OS turn response drifts', async () => {
    ...
    await expect(api.addTutorTurn(7, { content: 'I am stuck.' })).rejects.toBeInstanceOf(ApiValidationError);
  });
});
