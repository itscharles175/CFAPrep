...
describe('DiagnosticsPanel reliability console', () => {
  beforeEach(() => {
    ...
    mocks.useContentHealth.mockReturnValue(query({ score: 100, status: 'ok', warnings: [] }));
  ...
  it('surfaces release evidence and scheduler freshness in one panel', () => {
    ...
    expect(screen.getByText('recent benchmark and generation-quality evidence is present')).toBeInTheDocument();
  ...
  it('runs a scheduled task and refreshes trust evidence', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Run Daily local database backup' }));

    await waitFor(() => expect(mocks.runScheduledTask).toHaveBeenCalledWith('daily_backup'));
  ...
  it('surfaces runtime errors and opens exported logs', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Open reliability logs' }));

    await waitFor(() => {
      expect(mocks.exportBackendLogs).toHaveBeenCalled();
      expect(mocks.openPath).toHaveBeenCalledWith('C:\\Users\\charl\\LSATLab\\backend\\logs');
    });
  });
});
