...
function releaseFreshnessContract(detail: Record<string, unknown> | undefined): ReleaseFreshnessContract | null {
...
export function DiagnosticsPanel() {
  ...
  const [lastCrash, setLastCrash] = useState<LastCrash | null>(() => readLastCrash());
  ...
  const latestBackupName = stringOrNull(backupNewest?.name) ?? (list[0]?.name ? list[0].name : null);
  const backupAgeSeconds = numberOrNull(backupDetail?.newest_age_seconds);
  const providerName =
    stringOrNull(modelDetail?.provider) ?? o?.models?.local_provider ?? o?.models?.realtime_provider ?? 'unknown';
  const providerReachable =
    boolOrNull(modelDetail?.provider_reachable) ?? boolOrNull(modelDetail?.ollama_reachable) ?? null;
  ...
  const readinessRows = [
    ...
    },
    {
      ...
      detail: trustManifest ? `${trustManifest.score}/100` : 'Waiting for trust manifest',
    },
    {
      key: 'logs',
      label: 'Logs',
      ok: runtimeEvidence?.log_dir_writable ?? trustChecks.runtime_evidence?.status === 'ok',
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0">
        ...
        <Button size="sm" variant="ghost" onClick={refreshAllDiagnostics} aria-label="Refresh diagnostics">
      ...
      <CardContent className="space-y-4 text-sm">
        <div className="space-y-3 rounded-md border bg-muted/20 p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge
              variant={
                readinessRows.every((row) => row.ok) ? 'success' : recoveryActions.length ? 'warning' : 'outline'
              }
            >
            ...
            {readinessRows.map((row) => (
              <div key={row.key} className="rounded border bg-background/70 p-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant={row.ok ? 'success' : 'warning'} className="shrink-0">
                    {row.ok ? 'ok' : 'check'}
                  </Badge>
                </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="tabular-nums text-muted-foreground">
            {o?.last_coach_refresh_ms != null ? `${Math.round(o.last_coach_refresh_ms / 60000)}m ago` : '—'}
          </span>
        </div>
        <div className="flex items-center justify-between">
        ...
        <div className="flex items-center justify-between">
          ...
          {trustManifest?.status === 'blocked' ? (
            ...
            <Badge variant="warning">{trustManifest ? `${trustManifest.score}/100` : 'pending'}</Badge>
          )}
        </div>
        ...
        <div className="space-y-2 rounded-md border bg-muted/20 p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge variant={(runtimeEvidence?.recent_error_count ?? 0) > 0 ? 'warning' : 'success'}>
              {runtimeEvidence ? `${runtimeEvidence.recent_error_count} errors` : 'pending'}
            </Badge>
          </div>
          <div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">
            ...
            <span className="flex items-center gap-1">
              ...
              {runtimeEvidence ? `${runtimeEvidence.log_file_count} log files` : 'No runtime evidence'}
            </span>
          </div>
          {runtimeEvidence?.last_request_error ? (
            <div className="rounded-md bg-destructive/5 px-2 py-1.5 text-xs">
              <p className="font-medium text-destructive">Last request error</p>
              ...
              <p className="truncate text-muted-foreground">{runtimeEvidence.last_request_error.message}</p>
            </div>
            ...
            <div className="rounded-md bg-warning/10 px-2 py-1.5 text-xs text-warning">
              ...
              <p className="truncate">
                {[recentRuntimeErrors[0].status, recentRuntimeErrors[0].path].filter(Boolean).join(' · ') ||
                  recentRuntimeErrors[0].file}
                {': '}
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              <span className="text-xs tabular-nums text-muted-foreground">
                {trustManifest?.generated_at ? timeAgo(trustManifest.generated_at) : 'not generated'}
              </span>
            </div>
          </div>
          {releaseContract ? (
            <div className="grid gap-1.5 text-xs text-muted-foreground sm:grid-cols-3">
              <span>Live head: {shortSha(releaseContract.live_head) ?? 'unknown'}</span>
              <span>Blocking changes: {releaseContract.blocking_status_lines.length}</span>
              <span>Ignored policy files: {releaseContract.ignored_status_lines.length}</span>
              ...
              return (
                <div key={key} className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-muted-foreground">{check?.summary ?? 'No current evidence'}</p>
                  </div>
                  <Badge variant={trustVariant(check?.status)} className="shrink-0">
        ...
        {o?.cloud_tokens && o.cloud_tokens.total_tokens > 0 ? (
          <div className="flex items-center justify-between">
            ...
            <span className="tabular-nums text-muted-foreground">
              ...
              {o.cloud_monthly_budget_usd ? ` · budget $${o.cloud_monthly_budget_usd}` : ''}
            </span>
          </div>
        ) : null}
            ...
            {tasks.slice(0, 5).map((task) => (
              <div key={task.key} className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {task.status} · last {task.last_run_at ? timeAgo(task.last_run_at) : 'never'} · next{' '}
        ...
        {import.meta.env.DEV && (
          <details className="text-xs text-muted-foreground">
            ...
            <p className="mt-1">
              MCP (Jarvis): see <span className="font-mono">docs/15-mcp.md</span> — run{' '}
        ...
        {logDir ? (
          <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
            <p className="min-w-0">
              Desktop logs: <span className="break-all font-mono">{logDir}</span>
            </p>
            <Button
              size="sm"
              variant="ghost"
        ...
        <div className="space-y-1 border-t pt-3">
          ...
          {lastCrash ? (
            <div className="rounded-md bg-destructive/5 px-3 py-2 text-xs">
              <p className="font-medium text-destructive">{lastCrash.message}</p>
              <p className="mt-0.5 text-muted-foreground">
          ...
          {list.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {list.map((b) => (
                  <TableRow key={b.name}>
                    <TableCell className="max-w-[14rem] truncate font-mono text-xs">{b.name}</TableCell>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This replaces your current study database with <span className="font-mono">{restoreTarget}</span>. Recent
              attempts not in the snapshot will be lost, and a restart is recommended afterward.
            </DialogDescription>
          </DialogHeader>
