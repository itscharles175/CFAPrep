...
function timingCells(rows: { question_order: number; time_ms: number; is_correct: boolean; difficulty: number }[]): HeatCell[] {
  return rows.map((r) => ({
...
export function SessionCompare({ sessions }: { sessions: SessionSummary[] }) {
  ...
  if (list.length < 2)
    return (
      <Card className="shadow-e1">
        <CardHeader>
          <CardTitle className="text-base">Compare sessions</CardTitle>
          <CardDescription>
            Side-by-side timing strips for two finished sections.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ChartEmpty
            title="Need two finished sections"
            hint="Complete a second timed section to compare pacing side by side."
            height={140}
          />
        </CardContent>
      </Card>
  ...
  return (
    <Card className="shadow-e1">
      <CardHeader className="flex-row flex-wrap items-start justify-between gap-3 space-y-0">
        ...
        <Button
          ...
          onClick={() =>
            downloadCsv(
              "session-compare.csv",
              ["Session", "Q", "Seconds", "Result"],
              [
                ...rowsA.map((r) => ["A", String(r.question_order), String(Math.round(r.time_ms / 1000)), r.is_correct ? "Correct" : "Missed"]),
                ...rowsB.map((r) => ["B", String(r.question_order), String(Math.round(r.time_ms / 1000)), r.is_correct ? "Correct" : "Missed"]),
              ],
            )
          }
        >
      ...
      <CardContent className="space-y-4">
        ...
        {(ta.isLoading || tb.isLoading) && <SkeletonChart className="border-0 p-0 shadow-none" />}
          ...
          <>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                {rowsA.length ? <HeatStrip cells={timingCells(rowsA)} ramp="inferno" showScaleKey={false} /> : <p className="text-xs text-muted-foreground">No data</p>}
              </div>
              <div>
                <div className="mb-1 text-xs font-medium text-muted-foreground">Session B</div>
                {rowsB.length ? <HeatStrip cells={timingCells(rowsB)} ramp="inferno" showScaleKey={false} /> : <p className="text-xs text-muted-foreground">No data</p>}
              </div>
            </div>
            {/* One shared scale key for both columns. */}
            <ColorScaleKey
              className="mt-1"
