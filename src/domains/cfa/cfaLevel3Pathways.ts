export type Level3Pathway = 'portfolio-management' | 'private-markets' | 'private-wealth';
export type Level3PathwayClassification = Level3Pathway | 'core';

export const DEFAULT_LEVEL3_PATHWAY: Level3Pathway = 'portfolio-management';
...
export const LEVEL3_PATHWAY_TOPICS: Record<Level3Pathway, string> = {
  'portfolio-management': 'pm-pathway',
  'private-markets': 'private-markets-pathway',
  'private-wealth': 'private-wealth-pathway',
};

export interface Level3PathwayOption {
  value: Level3Pathway;
  label: string;
}

export const LEVEL3_PATHWAY_OPTIONS: Level3PathwayOption[] = [
...
const PATHWAY_BY_TOPIC: Record<string, Level3Pathway> = Object.fromEntries(
  Object.entries(LEVEL3_PATHWAY_TOPICS).map(([pathway, topicId]) => [topicId, pathway as Level3Pathway]),
);
const CORE_TOPIC_SET = new Set(LEVEL3_CORE_TOPIC_IDS);

export function normalizeLevel3Pathway(pathway: string | null | undefined = DEFAULT_LEVEL3_PATHWAY): Level3Pathway {
  return pathway && pathway in LEVEL3_PATHWAY_TOPICS ? (pathway as Level3Pathway) : DEFAULT_LEVEL3_PATHWAY;
}

export function level3PathwayTopicId(pathway: string | null | undefined = DEFAULT_LEVEL3_PATHWAY): string {
  return LEVEL3_PATHWAY_TOPICS[normalizeLevel3Pathway(pathway)];
}

export function level3PathwayForTopic(topicId: string | null | undefined): Level3PathwayClassification | null {
...
export function level3TopicBelongsToPathway(topicId: string | null | undefined, pathway: string | null | undefined = DEFAULT_LEVEL3_PATHWAY): boolean {
  const topicPathway = level3PathwayForTopic(topicId);
  return topicPathway === 'core' || topicPathway === normalizeLevel3Pathway(pathway);
}

export function level3PathwayFromStorage(storage: Storage | null | undefined = globalThis?.localStorage): Level3Pathway {
  try {
...
export function writeLevel3PathwayToStorage(pathway: string | null | undefined, storage: Storage | null | undefined = globalThis?.localStorage): Level3Pathway {
