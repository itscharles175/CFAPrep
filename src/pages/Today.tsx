...
export function summarizeTodayWorkload(plan: Pick<StudyPlan, 'actions'> | null | undefined): TodayWorkloadSummary {
  ...
  const totalMinutes = allDurationsKnown ? actions.reduce((sum, action) => sum + action.estimatedMinutes, 0) : null;
...
export default function Today() {
  ...
  const [lsatSidecarUnavailable, setLsatSidecarUnavailable] = useState(false);
  ...
  async function refresh() {
    ...
    try {
      const next =
        domain === 'cfa' ? await buildStudyPlan({ pathway: activePathway }) : createDomainFallbackPlan(domain, goal);
      setPlan(next);
    } finally {
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    ...
    nextPlan
      .then((next) => {
        ...
        try {
          if (typeof Notification !== 'undefined' && Notification.permission === 'granted' && next?.dueCount > 0) {
  ...
  // Before offering an LSAT launch from Today, make the local-runtime state
  // explicit. The host fallback plan remains available; this only prevents the
  // CTA from implying that the LSAT practice surface is ready when its sidecar
  // is not running.
  useEffect(() => {
    let active = true;
    if (domain !== 'lsat') {
      setLsatSidecarUnavailable(false);
      return () => {
        active = false;
      };
    }
    setLsatSidecarUnavailable(false);
    fetchLsatSidecarJson('/api/health', { timeoutMs: 1500 })
      .then((result) => {
        if (active) setLsatSidecarUnavailable(!result.reachable || !result.ok);
      })
      .catch(() => {
        if (active) setLsatSidecarUnavailable(true);
      });
    return () => {
      active = false;
    };
  ...
  const correctAnswerCount = drill.questions.filter(
    (question) => drillAnswers[question.id] === question.correct,
  ).length;
  useEffect(() => {
    ...
    try {
      ...
      if (!reading.chunks?.length) {
        setDrill({
          ...
          questions: [],
          error:
            'No ingested curriculum for this topic yet — import a .qvsource bundle or pick a folder of PDFs in System Health to ground the drill.',
        });
        return;
      }
  ...
  const sessionDomainLabel = sessionDomain ? DOMAIN_LABELS[sessionDomain] : null;
  const hasMismatchedFocusSession = Boolean(focusSession.session && !sameDomainSession);
  const heroTitle = top ? todayHeroTitle({ domain, title: top.title, sessionStatus, sessionDomain }) : null;
  ...
  const timerPanel = (
    <div className="qv-stack-3">
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
        ...
        >
          {focusSession.session?.status === 'running'
            ? hasMismatchedFocusSession
              ? `Focusing · ${sessionDomainLabel} session`
              : 'Focusing'
            : focusSession.session?.status === 'saving'
              ? hasMismatchedFocusSession
                ? `Saving · ${sessionDomainLabel} session`
                : 'Saving'
              : focusSession.session?.status === 'save-error'
                ? hasMismatchedFocusSession
                  ? `Save failed · ${sessionDomainLabel} session`
                  : 'Save failed'
                : focusSession.session
                  ? hasMismatchedFocusSession
                    ? `Paused · ${sessionDomainLabel} session`
                    : 'Paused'
                  : 'Ready'}
        </StatusBadge>
      </div>
      <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
        ...
        {hasMismatchedFocusSession && sessionDomain && (
          <button
            className="btn btn-primary btn-sm"
            onClick={() => updateStudyContext({ domain: sessionDomain })}
          >
            Switch to {sessionDomainLabel} to {focusSession.session?.status === 'paused' ? 'resume' : 'manage'}
          </button>
        )}
        {focusSession.session?.status === 'running' && !hasMismatchedFocusSession && (
          <>
            <button className="btn btn-secondary btn-sm" onClick={focusSession.pause}>
              Pause
            </button>
            <button className="btn btn-secondary btn-sm" onClick={focusSession.stopAndSave}>
              Stop &amp; log
            </button>
          </>
        )}
        {focusSession.session?.status === 'paused' && !hasMismatchedFocusSession && (
          <>
            <button className="btn btn-primary btn-sm" onClick={() => focusSession.start()}>
              Resume
            </button>
            <button className="btn btn-secondary btn-sm" onClick={focusSession.stopAndSave}>
              Stop &amp; log
        ...
        {focusSession.session?.status === 'save-error' && !hasMismatchedFocusSession && (
          <>
            <button className="btn btn-primary btn-sm" onClick={focusSession.retrySave}>
              Retry log
            </button>
            <button className="btn btn-secondary btn-sm" onClick={() => focusSession.start()}>
              Keep studying
            </button>
            <button className="btn btn-secondary btn-sm" onClick={focusSession.discard}>
              Discard
            </button>
          </>
        )}
      ...
      {focusSession.session?.status === 'save-error' && (
        <p role="alert" className="qv-text-danger qv-fs-sm qv-m-0">
          Your session is still saved as a local checkpoint.{' '}
          {focusSession.session.saveError || 'Retry when the vault is available.'}
        </p>
      )}
    ...
    <div className="qv-stack-3">
      <div className="flex-between qv-row-3" style={{ gap: 'var(--space-3)', alignItems: 'flex-start' }}>
        ...
        <button
          ...
          style={{ flexShrink: 0 }}
        >
          {narrative.state === 'loading' ? (
            'Thinking…'
          ) : narrative.text ? (
            'Regenerate narrative'
          ) : (
            <>
              <Sparkles size={14} aria-hidden="true" /> Why this plan
            </>
          )}
        </button>
      </div>
      {narrative.state === 'done' && narrative.text && (
        <p className="qv-callout qv-m-0" style={{ whiteSpace: 'pre-line' }}>
          {narrative.text}
        </p>
      )}
      {narrative.state === 'error' && (
        <p className="qv-text-danger qv-fs-sm qv-m-0" role="alert" aria-live="assertive">
          {narrative.error}
        </p>
      )}
    </div>
    ...
    <div className="qv-stack-2">
      <div className="flex-between qv-row-3-start" style={{ gap: 'var(--space-3)' }}>
        ...
        <button
          ...
          title={
            journalSaveError ||
            (journal.savedAt ? `Last saved ${new Date(journal.savedAt).toLocaleTimeString()}` : 'Save')
          }
          style={{ flexShrink: 0 }}
        >
      ...
      {journalSaveError && (
        <p id="today-journal-error" role="alert" aria-live="assertive" className="qv-text-danger qv-fs-sm qv-m-0">
          Could not save this reflection. {journalSaveError} Your text is still here; retry when the local vault is
          available.
        </p>
      )}
    </div>
    ...
    <div className="page-container today-page">
            ...
            <>
              ...
              {plan.dueCount > 0 && (
                <StatusBadge tone="warning">
                  {plan.dueCount} review{plan.dueCount === 1 ? '' : 's'} due
                </StatusBadge>
              )}
              {plan.weakCount > 0 && (
                <StatusBadge tone="danger">
                  {plan.weakCount} weak topic{plan.weakCount === 1 ? '' : 's'}
                </StatusBadge>
              )}
              {plan.peakReviewDay && (
                <StatusBadge tone="exam">
                  Peak {plan.peakReviewDay.date} · {plan.peakReviewDay.count}
                </StatusBadge>
              )}
              {examCountdown && (
      ...
      {!plan ? (
        ...
        <div role="status" aria-busy="true" aria-label="Loading today’s plan">
          <p className="muted-copy qv-m-0" aria-live="polite">
        ...
        <>
          {domain === 'lsat' && lsatSidecarUnavailable && (
            <Surface tone="study" status="warning" density="compact" className="today-lsat-runtime-note">
              <div className="qv-row-3" style={{ alignItems: 'flex-start' }}>
                <StatusBadge tone="warning">LSAT local service unavailable</StatusBadge>
                <p className="muted-copy qv-m-0 qv-fs-sm">
                  Start the LSAT backend before opening practice. Your host study plan is still available.
                </p>
              </div>
            </Surface>
          )}
          ...
          <OnboardingResume onResume={() => navigate('/')} onResumeSession={(path) => navigate(path)} />
          ...
          <div className="today-disclosures">
            <details className="today-disclosure">
              ...
              <div className="today-disclosure__body today-session-tools">
                <section aria-labelledby="today-timer-title">
                  <h3 id="today-timer-title">
                    <Clock size={17} aria-hidden="true" /> Focus timer
                  </h3>
                  {timerPanel}
                </section>
                <section aria-labelledby="today-journal-title">
                  <h3 id="today-journal-title">
                    <NotebookPen size={17} aria-hidden="true" /> Journal
                  </h3>
                  {journalPanel}
                </section>
              </div>
            ...
            <details className="today-disclosure">
              <summary>
                <span>Full plan</span>
                <span className="today-disclosure__meta">{workload.totalLabel}</span>
              </summary>
                ...
                {weakAction && (
                  <Surface tone="study" status="warning">
                    <div className="flex-between qv-row-3-start">
                      <div>
                        <StatusBadge tone="warning">
                          <Sparkles size={14} /> Drill your weakest topic
                        </StatusBadge>
                        <h3 className="qv-mt-2" style={{ marginBottom: 0 }}>
                          {weakAction.title}
                        </h3>
                      ...
                      <button
                        ...
                        title={
                          weakTopic ? 'Generate 3 grounded MCQs via the local LLM' : 'Topic path could not be parsed'
                        }
                      >
                        {drill.state === 'loading'
                          ? 'Generating…'
                    ...
                    {drill.state === 'error' && (
                      <p
                        className="qv-text-danger qv-mt-2"
                        role="alert"
                        aria-live="assertive"
                    ...
                    {drill.questions.length > 0 &&
                      (() => {
                        ...
                        const correct = drill.questions.filter((q) => drillAnswers[q.id] === q.correct).length;
                        return (
                          <div className="qv-stack-3 qv-mt-3">
                            ...
                            {drill.questions.map((question, qi) => {
                              ...
                              return (
                                <div key={question.id} className="qv-card">
                                  <strong>
